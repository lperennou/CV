%-------------------------------------------------------------------------------
%	SECTION TITLE
%-------------------------------------------------------------------------------
\cvsection{Communications in Conferences}

%-------------------------------------------------------------------------------
%	CONTENT
%-------------------------------------------------------------------------------
\begin{cventries}

%---------------------------------------------------------
  \cventry
    {} % Role
    {} % Title
    {} % Location
    {} % Date(s)
    {
      \vspace{-.5cm}
      \begin{cvitems} % Description(s)
      \item {\textit{Workload Characterization for a Non-Hyperscale Public Cloud Platform }\\ 
	|~~~~ Loïc Pérennou, Mar Callau-Zori, Sylvain Lefebvre, Raja Chiky. 12th IEEE International Conference on Cloud Computing, 2019}
      \item {\textit{Applying Supervised Machine Learning to Predict Virtual Machine Runtime }\\
	  |~~~~ Loïc Pérennou, Raja Chiky. 11th International Conference on Computational Collective Intelligence, 2019} 
	\item {\textit{Runtime Prediction Error Levels for Virtual Machine Placement in IaaS Cloud }\\
	  |~~~~ Loïc Pérennou, Sylvain Lefebvre. 9th International Conference on Ambient Systems, Networks and Technologies, 2018 }
      \end{cvitems}
    }

%---------------------------------------------------------
\end{cventries}

